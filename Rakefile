...
desc "Build dev website"
task :dev do
  message "~~~ Building devsite ~~~"
  devsite = true
...
task :html do
  ...
  if !which("jekyll")
    ...
    if devsite
      out = `jekyll build --plugins ./_plugins --config _config.yml,_config.dev.yml 2>&1`
    else
...
task :css do
  ...
  if !which("compass")
    ...
    out += `sed -i -e 's/src\\/img/img/g' src/css/site.css 2>&1`
    print "\n" + out
    message "done".colorize(:green) + '.'
  end
end
...
task :pdiff do
  ...
  Dir.chdir dir do
    # site_diff
    pdiff_options = [
      "--upload_build_id=#{SECRETS['dpxdt']['build_id']}",
      ...
      "--crawl_depth=-1",
      "--inject_css='.kcago-ship,.kcago-bullet{display:none}.tag-cloud-tags{text-indent:-32000em}'"
    ]
    out = `. ~/git/dpxdt/common.sh; ~/git/dpxdt/dpxdt/tools/site_diff.py #{pdiff_options.join(' ')} #{DEV_CONFIG['url']} #{DEV_CONFIG['url']}/blog/ 2>&1`
    # url_pair_diff
