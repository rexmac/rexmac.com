 ...
 *
 * Heavily based on https://github.com/lrgalego/jquery-treemap
 * and http://www.jnathanson.com/blog/client/jquery/heatcolor/jquery.heatcolor.0.0.1.js
 */
  ...
  methods = {
    init: function(options) {
      ...
      return this.each(function() {
        var $el = $(this),
            $div, r, nodes, i, n, node, $node, min, max, color, $legend;
        ...
        if(undefined === settings.data) return;
        ...
        color = [helpers.hex2hsb(settings.color[0]), helpers.hex2hsb(settings.color[1])];
        ...
        for(i = 0, n = nodes.length, min = nodes[n-1].value, max = nodes[0].value; i < n; ++i) {
          ...
          $node.css($.extend(node.bounds.style(), {
            'position': 'absolute',
            'backgroundColor': '#' + helpers.rgb2hex(helpers.hsb2rgb(helpers.lerpHsb(node.value, max, color[0], color[1])))
          }));
          $node.appendTo($div);
        }
        ...
        if(settings.legend) {
          ...
          if(settings.legend.labels) {
            if(settings.legend.labels.low) {
              $legend.append($('<span/>').addClass('treemap-legend-begin').text(settings.legend.labels.low))//.css('backgroundColor', settings.color[0]))
            }
            if(settings.legend.labels.high) {
              $legend.append($('<span/>').addClass('treemap-legend-end').text(settings.legend.labels.high))//.css('backgroundColor', settings.color[1]))
            }
          }
          if(settings.legend.position && settings.legend.position === 'top') {
